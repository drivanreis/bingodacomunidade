...
class UsuarioAdministrativo(Base):
    ...
    def __repr__(self):
        return (
            f"<UsuarioAdministrativo(id={self.id}, login={self.login}, nivel={self.nivel_acesso})>"
        )
...
class UsuarioLegado(Base):
    ...
    token_verificacao_email = Column(
        String(100), nullable=True, index=True
    )  # Token para verificar email
    token_verificacao_expiracao = Column(
...
class Sorteio(Base):
    ...
    max_cards = Column(
        Integer, nullable=True, comment="Limite máximo de cartelas vendidas (null = ilimitado)"
    )
    ...
    # Status
    status = Column(
        SQLEnum(StatusSorteio), nullable=False, default=StatusSorteio.AGENDADO, index=True
    )
...
class Cartela(Base):
    ...
    # Status
    status = Column(
        SQLEnum(StatusCartela), nullable=False, default=StatusCartela.NO_CARRINHO, index=True
    )
...
class Feedback(Base):
    ...
    # Status do feedback
    status = Column(
        SQLEnum(StatusFeedback), nullable=False, default=StatusFeedback.PENDENTE, index=True
    )
...
class SistemaAuditoria(Base):
    ...
    iniciado_em = Column(
        DateTime(timezone=True), nullable=False, comment="Primeira inicialização registrada"
    )
    ultima_inicializacao_em = Column(
        DateTime(timezone=True), nullable=False, comment="Última inicialização registrada"
    )
