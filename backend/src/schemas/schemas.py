...
def validate_nome_completo(v: Optional[str]) -> Optional[str]:
    ...
    if re.search(
        r"['\";]|--|/\*|\*/|<\s*script|\b(or|select|drop|insert|delete|update)\b",
        nome,
        re.IGNORECASE,
    ):
        raise ValueError("Nome Completo invalido")
...
class FirstAccessSetupRequest(BaseModel):
    ...
    nome: str = Field(
        ..., min_length=3, max_length=200, description="Nome completo do desenvolvedor"
    )
...
class UpdateProfileRequest(BaseModel):
    ...
    whatsapp: Optional[str] = Field(None, description="WhatsApp (+55DDNNNNNNNNN)")
    chave_pix: Optional[str] = Field(None, description="Chave PIX para receber prêmios")
    senha_atual: Optional[str] = Field(
        None, description="Senha atual (obrigatória para trocar senha)"
    )
...
class SorteioBase(BaseModel):
    ...
    def validate_rateio_sum(self):
        ...
        rateio_total = (
            self.rateio_premio + self.rateio_paroquia + self.rateio_operacao + self.rateio_evolucao
        )
...
class SignupFielRequest(BaseModel):
    ...
    whatsapp: str = Field(
        ..., description="WhatsApp para notificações de prêmios (DDD + número local)"
    )
    ...
    device_fingerprint: Optional[str] = Field(
        None, max_length=128, description="Fingerprint do dispositivo para antifraude"
    )
    senha: str = Field(..., min_length=6, description="Senha (mínimo 6 caracteres)")
...
class CreateAdminSiteRequest(BaseModel):
    ...
    nome: Optional[str] = Field(
        None, min_length=3, max_length=120, description="Nome/apelido do administrador"
    )
    senha: str = Field(..., min_length=6, description="Senha (mínimo 6 caracteres)")
    ...
    telefone: str = Field(
        ..., min_length=10, max_length=20, description="Telefone para SMS/WhatsApp (fator 2)"
    )
...
class CreateAdminParoquiaRequest(BaseModel):
    ...
    nome: str = Field(
        ..., min_length=3, max_length=200, description="Nome do novo administrador paroquial"
    )
    login: str = Field(
        ..., min_length=3, max_length=100, description="Login único para o administrador"
    )
    senha: str = Field(..., min_length=6, description="Senha (mínimo 6 caracteres)")
...
class SetAdminSitePasswordRequest(BaseModel):
    ...
    nova_senha: str = Field(
        ..., min_length=6, max_length=64, description="Nova senha para o administrador alvo"
    )
