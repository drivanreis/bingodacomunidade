"""
===========================================================================
TPIC - Integração com Groq API para Análise Visual
===========================================================================
SUPER RÁPIDO, GRÁTIS e SEM CARTÃO DE CRÉDITO!
"""
...
logger = Logger("groq_vision")

class GroqVisionAnalyzer:
    """
    Analisa screenshots usando Groq API (GRÁTIS + RÁPIDO!).
    Usa modelos de código aberto (Llama, Mixtral).
    """
    
    def __init__(self):
        api_key = os.getenv("GROQ_API_KEY")
        if not api_key:
            raise ValueError("GROQ_API_KEY não está definida em .env")
        
        self.client = Groq(api_key=api_key)
        self.model = "llama-3.3-70b-versatile"  # Modelo ativo e gratuito
        self.conversation_history = []
        logger.success("Groq Vision Analyzer inicializado (SUPER RÁPIDO! 🚀)")
    ...
    def analyze_screenshot(
        ...
        """
        Analisa um screenshot usando Groq.
...
Analise esta screenshot de uma aplicação web baseado na imagem em base64.
...
Imagem em base64: {image_data[:100]}...

IMPORTANTE:
- Retorne APENAS JSON válido
- Identifique campos de formulário
- Detecte erros de validação
- Se houver modal, identifique
- Recomende a próxima ação"""
...
{{
    ...
    "evidence": "descrição do que vê na tela",
...
Retorne JSON:
{
    ...
    "critical": true/false,
    "affected_functionality": "funcionalidade afetada",
